...
let sockets = [];
let groups = new Map(); // Map of groupId -> { name: string, members: Set of socket IDs }
    ...
    message: (ws, message, isBinary) => {
      ...
      switch (clientMsg.type) {
        ...
        case MESSAGE_ENUM.GROUP_MESSAGE:
          ...
          if (groups.has(groupId) && groups.get(groupId).members.has(ws.id)) {
            serverMsg = {
        ...
        case MESSAGE_ENUM.GROUP_IMAGE_MESSAGE:
          ...
          if (groups.has(imageGroupId) && groups.get(imageGroupId).members.has(ws.id)) {
            serverMsg = {
        ...
        case MESSAGE_ENUM.CREATE_GROUP:
          console.log('Creating new group for user:', ws.username);
          const newGroupId = uuid();
          console.log('New group ID:', newGroupId);
          
          if (!groups.has(newGroupId)) {
            groups.set(newGroupId, {
              name: clientMsg.groupName || `Group ${newGroupId.slice(0, 8)}`,
              members: new Set([ws.id])
            });
            if (!userGroups.has(ws.id)) {
              userGroups.set(ws.id, new Set());
            }
            userGroups.get(ws.id).add(newGroupId);
            ws.subscribe(newGroupId);
            
            console.log('Group created successfully');
            console.log('Current groups:', Array.from(groups.entries()));
            ...
            serverMsg = {
              ...
              groups: Array.from(groups.entries()).map(([id, group]) => ({
                id: id,
                name: group.name,
                members: Array.from(group.members).map(socketId => 
                  sockets.find(s => s.id === socketId)?.username
                ).filter(Boolean)
              }))
            };
            console.log('Sending group list update:', serverMsg);
            app.publish(MESSAGE_ENUM.GROUP_LIST, JSON.stringify(serverMsg));
          } else {
            console.error('Group ID collision detected:', newGroupId);
          }
          break;

        case MESSAGE_ENUM.GROUP_LIST:
          console.log('Sending group list to client');
          const groupList = Array.from(groups.entries()).map(([id, group]) => ({
            id: id,
            name: group.name,
            members: Array.from(group.members).map(socketId => 
              sockets.find(s => s.id === socketId)?.username
          ...
          console.log('Group list to send:', groupList);
          serverMsg = {
            type: MESSAGE_ENUM.GROUP_LIST,
            groups: groupList
          };
        ...
        case MESSAGE_ENUM.JOIN_GROUP:
          ...
          if (groups.has(joinGroupId)) {
            groups.get(joinGroupId).members.add(ws.id);
            if (!userGroups.has(ws.id)) {
              userGroups.set(ws.id, new Set());
            }
        ...
        case MESSAGE_ENUM.LEAVE_GROUP:
          ...
          if (groups.has(leaveGroupId)) {
            groups.get(leaveGroupId).members.delete(ws.id);
            if (userGroups.has(ws.id)) {
        ...
        case MESSAGE_ENUM.GROUP_TYPING_STARTED:
          ...
          if (groups.has(typingGroupId) && groups.get(typingGroupId).members.has(ws.id)) {
            serverMsg = {
        ...
        case MESSAGE_ENUM.GROUP_TYPING_STOPPED:
          ...
          if (groups.has(stopTypingGroupId) && groups.get(stopTypingGroupId).members.has(ws.id)) {
            serverMsg = {
        ...
        case MESSAGE_ENUM.GROUP_REPLY_MESSAGE:
          ...
          if (groups.has(replyGroupId) && groups.get(replyGroupId).members.has(ws.id)) {
            serverMsg = {
      ...
      if (userGroups.has(ws.id)) {
        userGroups.get(ws.id).forEach(groupId => {
          if (groups.has(groupId)) {
            groups.get(groupId).members.delete(ws.id);
          }
        });
